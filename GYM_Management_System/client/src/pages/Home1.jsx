...
export default function Home() {
  ...
  const handleAddToCart = async (itemId) => {
    try {
      ...
      // Check if the item quantity is greater than 0 before adding to cart
...
// Send request to update item quantity in the backend
const responseUpdate = await fetch(`http://localhost:3000/api/items/updateitem/${itemId}`, {
  method: "PUT",
  headers: {
    "Content-Type": "application/json",
  },
  ...
  throw new Error("Failed to update item quantity");
      ...
      const response = await fetch(`http://localhost:3000/api/items/Cart`, {
        ...
        body: JSON.stringify({
          ...
          image: selectItem.image,
          price: selectItem.price,
          quantity: selectItem.quantity, // Update quantity here
          Description: selectItem.Description,
        }),
      });
